...
class Battle {
	...
	BattleMission@ join(AI& ai, War& war, FleetAI@ flAI) {
		...
		if(defendPlanet !is null)
			...
			@moveTo = eliminate;
		@mission.move = war.movement.move(flAI.obj, moveTo, MP_Critical, spread=true, nearOnly=true);//nearOnly=True
	...
	bool tick(AI& ai, War& war, double time) {
		...
		for(uint i = 0, cnt = fleets.length; i < cnt; ++i) {
			...
			if(miss.arrived) {
				...
				if(miss.fleet.supplies < 0.1)//Illyia - Changed from 0.05, better retreat {
					if(isCapturingAny && eliminate is null)
						shouldRetreat = true;
					else if(ourStrength < enemyStrength * 0.75)
						shouldRetreat = true;
				}
				if(miss.fleet.fleetHealth < 0.65)//Illyia- Changed from 0.25 better retreat {
		...
		if(!inCombat && (anyArrived || !isAttack)) {
			...
			if(gameTime > lastCombat + 30.0)//changed from 90, seems to make AI more responsive with fleets? {
				for(uint i = 0, cnt = fleets.length; i < cnt; ++i)
			...
			if(battles.length < ai.behavior.maxBattles && relations.isFightingWar(aggressive=true)
				...
				for(uint i = 0, cnt = intelligence.intel.length; i < cnt; ++i) {
					...
					for(uint n = 0, ncnt = intel.theirBorder.length; n < ncnt; ++n) {
						...
						check(sys, 2.0);//Illyia change from 1.0, helps enable attacking behind border
					}//Illyia Insertion start -
					for(uint n = 0, ncnt = intel.theirOwned.length; n < ncnt; ++n) {
						auto@ sys = intel.theirOwned[n];
						check(sys, 4.0);
					}//illyia insertion end helps enable attacking behind border
				}
